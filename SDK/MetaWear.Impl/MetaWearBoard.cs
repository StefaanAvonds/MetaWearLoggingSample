...
namespace MbientLab.MetaWear.Impl
{
    ...
    public class MetaWearBoard : IMetaWearBoard
    {
        ...
        internal static int COMMS_TIMEOUT = 5000;
        ...
        private class Timer : ModuleImplBase, IModule
        {
            ...
            internal Task<DataTypeBase> create(uint period, ushort repititions, bool delay)
            {
                ...
                timeoutFuture = new System.Threading.Timer(e => createTimerTask.SetException(new TimeoutException("Scheduling task timed out")), null, MetaWearBoard.COMMS_TIMEOUT * 3, Timeout.Infinite);
        ...
        public async Task InitializeAsync(Action<float, string> progressCallback)
        {
            gatt.GetAdvertisementCharacteristics();
            ...
            if (persistent.attributes == null)
            {
                ...
                if (progressCallback != null)
                    progressCallback(0.1f, "Loaded Attributes");
            }
            ...
            if (progressCallback != null)
                progressCallback(0.15f, "Checking Versions");
            ...
            if (progressCallback != null)
                progressCallback(0.20f, "Checking Versions");
            ...
            try
            {
                ...
                if (progressCallback != null)
                    progressCallback(0.25f, "Discovering Modules");
                ...
                if (persistent.attributes.firmware == null || persistent.attributes.firmware.CompareTo(firmware) != 0)
                {
                    ...
                    await DiscoverModulesAsync(true, progressCallback);
                }
                else
                {
                    await DiscoverModulesAsync(false, progressCallback);
                }
        ...
        private async Task DiscoverModulesAsync(bool refresh, Action<float, string> progressCallback)
        {
            if (refresh)
            {
                persistent.id = 0;
            ...
            foreach (Module module in modules)
            {
                if (refresh || !persistent.attributes.moduleInfo.ContainsKey(module))
                {
                    ...
                    if (progressCallback != null)
                    {
                        progressCallback(0.25f + (++i * incr), "Loading Module: " + info.id);
                    }
                }
            }
        }
